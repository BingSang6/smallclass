...
(function () {
  ...
  function bank() { return banks[subject] || []; }

  function loadBank(done) {
    if (banks[subject] && banks[subject].length) return done();
    fetch(Store.SUBJECTS[subject].bank)
      .then(r => r.json())
      .then(j => { banks[subject] = j; done(); })
  ...
  function answer(stu, val) {
    ...
    if (ok) {
      ...
      Store.updateCurrent(s => {
        const sp = Store.subj(s, subject);
        sp.wrongPool = (sp.wrongPool || []).filter(id => id !== cur.id);
        sp.wrongPool.push(cur.id);
        if (sp.wrongPool.length > 30) sp.wrongPool.shift();
        sp.tagStreaks[tag] = 0;
      });
    }
